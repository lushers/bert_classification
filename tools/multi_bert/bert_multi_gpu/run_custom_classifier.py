...
class DataProcessor(object):
    ...
    @classmethod
    def _read_local_tsv(cls, input_file, quotechar=None):
        """Reads a tab separated value file."""
        lists = []
...
class MineABProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        return self._create_examples(
            self._read_local_tsv(os.path.join(data_dir, 'train.shuf')), 'train')

    def get_dev_examples(self, data_dir):
        """Gets a collection of `InputExample`s for the dev set."""
        return self._create_examples(
            self._read_local_tsv(os.path.join(data_dir, "eval_shuf.tsv")), 'dev')

    def get_test_examples(self, data_dir):
        """Gets a collection of `InputExample`s for prediction."""
        return self._create_examples(
            self._read_local_tsv(os.path.join(data_dir, "test.tsv")), 'test')
...
def main(_):
    ...
    if FLAGS.use_gpu and int(FLAGS.num_gpu_cores) >= 2:
        ...
        dist_strategy = tf.contrib.distribute.MirroredStrategy(
            ...
            #cross_tower_ops=tf.contrib.distribute.AllReduceCrossTowerOps('nccl',
            #                                                             num_packs=FLAGS.num_gpu_cores)
            cross_tower_ops=tf.contrib.distribute.AllReduceCrossTowerOps('hierarchical_copy'),
        )
        log_every_n_steps = 8
